...
import { useGetVolunteersQuery } from "../../Store/services/volunteer";
...
export default function VolunteerPage() {
  ...
  const { data, error, isLoading, isError } = useGetVolunteersQuery({
    page,
    name,
  ...
  //   return (
  //     <Box mt={5} textAlign="center">
  //       <Text color="red.500">{t("volunteerLoadError")}</Text>
  //       <Text fontSize="sm">{JSON.stringify(error)}</Text>
  //     </Box>
  //   );
  // }
  ...
  return (
    <AdsLayout>
      ...
      <Box mt={6} mb={8} textAlign="center">
        ...
        >
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                if (searchName.trim()) {
                  setSearchParams({ page: "1", name: searchName.trim() });
                } else {
        ...
        <Flex mt={4} gap={3} justify="center" align="center" wrap="wrap" px={4}>
          {(categoriesData?.results || categoriesData || []).length > 0 && (
            <Flex
              mt={4}
              gap={3}
              justify="center"
            ...
            >
              ...
              {(categoriesData?.results || categoriesData || []).map((cat) => {
                ...
                return (
                  <Button
                    ...
                    border="2px solid"
                    borderColor="var(--main-color)"
                    boxShadow="0 6px 16px rgba(72, 187, 120, 0.18)"
                    fontWeight="700"
                    _hover={{
                      bg: isSelected
                        ? "green.600"
                    ...
                    onClick={() => {
                      ...
                      if (isSelected) {
                        ...
                        newCategories = [...selectedCategories, String(cat.id)];
                      }
                      ...
                      if (newCategories.length > 0) {
                        params.category = newCategories.join(",");
                      }
      ...
      <Flex gap="10px" wrap="wrap" justify="center" align="center" mb={6}>
        {data?.results?.map((item) => (
          <VolunteerCard key={item.id} data={item} />
        ))}
      </Flex>
